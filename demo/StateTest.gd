...
var messages := [
	...
	"The ~[[h]~ or ~[[hold]~ waits until advance() is called. (Press any key.)[h] Then it shows more text.[h] The indicator appears.[h] Letting the user know they can take action.",
	"The ~[[w]~ or ~[[wait]~ pauses for a seconds.[w] Useful for[w]...[w] Dramatic tension?",
	"The ~[[p]~ or ~[[pace]~ controls animation speed.[p=0.25] Slow text down...[w][p=1.6] Or speed text up! Look how fast we are speaking![p=0.1] Down to a crawl![p=3.0] Or nearly instant!",
	"[p=0.5]The ~[[skip]~ tag... [skip]displays a chunk all at once.[;w=2] Good for [skip]making an impact[]!",
	"[p=0.5]The ~[[$expression]~ tags can trigger expressions at a point.[$print('Wee woo');$hop();w] Now let's wait till the end and then...[w][$print('The End');$hop();w]",
